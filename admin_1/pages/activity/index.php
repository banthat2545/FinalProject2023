...
<body class="hold-transition sidebar-mini">
    ...
    <script>
                ...
                data.response.forEach(function(item, index) {
                    tableData.push([
                        ...
                        `<div class="btn-group" role="group">
                            <a href="form-checkName.php?act_id=${item.act_id}" class="btn btn-success">
                                <i class="fas fa-check"></i> เช็คชื่อ
                            </a>
                        </div>`,
                        `<div class="btn-group" role="group">
                            <a href="info2.php?act_id=${item.act_id}" class="btn btn-info">
                                <i class="fas fa-search"></i> ดูข้อมูล
                            </a>
                            <button type="button" class="btn btn-danger" id="delete" data-id="${item.act_id}" data-index="${index}">
        ...
        $(function() {
            $.ajax({
                type: "GET",
                url: "../../service/activity/index2.php"
            }).done(function(data) {
                let tableData = []
                data.response.forEach(function(item, index) {
                    tableData.push([
                        ...
                        item.act_credit,
                        `<div class="btn-group" role="group">
                            <a href="info.php?act_id=${item.act_id}" class="btn btn-info">
                                <i class="fas fa-search"></i> ดูข้อมูล
                            </a>
                        </div>`
                    ])
                })
                initDataTables2(tableData)
            }).fail(function() {
                Swal.fire({
                    text: 'ไม่สามารถเรียกดูข้อมูลได้',
                    icon: 'error',
            ...
            function initDataTables2(tableData) {
                $('#logs2').DataTable({
                    data: tableData,
                    columns: [{
                            title: "ลำดับ",
